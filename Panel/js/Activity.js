...
$(document).ready(function(){
    ...
    function get_all_foodcatogry(){
        $.ajax({
            ...
            data : {email:"4@g.com",pass:"4",offset:"0",limit:"1000"},
            success: function(jd){
                recive_data=jd;
                if(jd.code=='200'){
                    for (i = 0; i <jd.result.length; i++) { 
                        ...
                        serv_item.find(".exit2").on("click", function() {
                            //delete
                            if(action_mod==2){
                                if(!confirm("Are you sure deleting activity"))
                                    return;
                                event.preventDefault();
                                const datas = new FormData();
                                datas.append('email',"4@g.com");
                                datas.append('pass',"4");
                                datas.append('activityId',item_selected);  
                                $("#cancel").hide();
                                $(".edit").hide();
                                $("#circle-loader").show(); 
                                setTimeout(function() 
                                {
                                    $.ajax({
                                        type: 'POST',
                                        url: 'http://medvisit.ir/screenMe/api/public/deleteActivities',
                                        data : datas,
    ...
    //side nav
    $("#activity_itm_shadow").addClass( "item_selected_shadow" );
    ...
    function show_sidenav(){
        ...
        $(".txt").fadeIn(500);
        $("#activity_itm_shadow").removeClass( "item_selected_shadow" ); 
    ...
    function editing(categoryid,categoryname,categorypic,fileupload){
        ...
        datas.append('email',"4@g.com");
        datas.append('pass',"4");
        datas.append('activityId',categoryid);
        datas.append('activityName',categoryname);
        datas.append('activityPic',categorypic);
        datas.append('uploaded_file',fileupload);
        setTimeout(function() 
        {  
            $.ajax({
                type: 'POST',
                url: 'http://medvisit.ir/screenMe/api/public/editActivities',
                data : datas,
                enctype: 'multipart/form-data',
                processData: false, 
                contentType: false,
    ...
    $("#save").on("click", function() {
        ...
        //add
            ...
            if(uploaded_file==null){
                ...
                alert("add title for new activity");
                return;
            }
           $("#save").hide();
            $("#cancel").hide();
            ...
            datas.append('activityName',new_food_title);
            datas.append('uploaded_file',uploaded_file);  
            setTimeout(function() 
            {  
                $.ajax({
                    type: 'POST',
                    url: 'http://medvisit.ir/screenMe/api/public/insertActivities',
                    data : datas,
                    enctype: 'multipart/form-data',
                    processData: false,  
                    contentType: false, 
