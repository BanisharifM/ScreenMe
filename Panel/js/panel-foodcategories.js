          ...
          for (i = 0; i < jd.result.length; i++) {
            ...
            var serv_item = $(
              '<div class="col-sm-4" id="' +
                ...
                '" name="' +
                food_id +
                '"><div class="serv-item text-center" name="' +
                food_id +
                '"><input  id="upload-img" class="upload-img" type="file" accept="image/*" style="display: none" name="' +
                food_id +
                '" /><img class="serv-icon" id="output' +
                food_id +
                '"  name="' +
                jd.result[i].pic +
                '" /><div class="bb" name="' +
                food_id +
                '"><i class=" animated infinite swing fas fa-times-circle exit2" name="' +
                food_id +
                '"></i><textarea rows="1"  id="serv-title' +
                food_id +
                '" class="box-input serv-title message-box" autocomplete="off"></textarea></div></div></div>'
            );
            ...
            serv_item
              ...
              .css({
                "background-image":
                  "url(./img/corner-" + arr_samp[index] + ".png)",
              });

            if ((i + 1) % 3 == 0) {
              var m = Math.max(
                $("#serv-title" + jd.result[i - 1].id).get(0).scrollHeight,
                $("#serv-title" + jd.result[i - 2].id).get(0).scrollHeight,
                $("#serv-title" + jd.result[i].id).get(0).scrollHeight
              );
              $("#serv-title" + jd.result[i - 2].id).css(
                "min-height",
                m + "px"
              );
              $("#serv-title" + jd.result[i - 1].id).css(
                "min-height",
                m + "px"
              );
              $("#serv-title" + jd.result[i].id).css("min-height", m + "px");
            } else if (i >= jd.result.length - (jd.result.length % 3)) {
              $("#serv-title" + jd.result[i].id).css(
                "min-height",
                $("#serv-title" + jd.result[i - 1].id).get(0).scrollHeight +
                  "px"
              );
            ...
            $(".upload-img").change(function () {
              if (action_mod == 1) item_selected = "-2";
            ...
            serv_item.find(".exit2").on("click", function () {
              //delete
              if (action_mod == 2) {
                if (!confirm("Are you sure deleting food category")) return;
  ...
  $("#iico2").click(function () {
    if (food_editable != 1 && food_adding != 1) {
      ...
      if (food_deletable == 1) {
        ...
        $("#saved").text("Deleted");
        $("#cancel")
          .removeClass("cancel-buttom")
          .addClass("saved-buttom")
          .show();
      } else {
        $("#firstiico")
        ...
        $("#panel-edit-status").text("");
        $("#cancel")
          .removeClass("saved-buttom")
          .addClass("cancel-buttom")
          .hide();
      }
    }
  });
    ...
    if (action_mod == 0) {
      ...
      for (i = 0; i < recive_data.result.length; i++) {
        ...
        if (temp_img_name == -5) {
          ...
          change_flag = 1;
        } else temp_changico = 0;
        if (temp_item_name != recive_data.result[i].name) change_flag = 1;
